 ...
 * mgist_mobilitydb.C 
 *
 * Multi Entry R-Tree for tgeompoint using Multi-Entry GiST
 *
...
#define MGIST_EXTRACT_BOXES_DEFAULT    5
#define MGIST_EXTRACT_BOXES_MAX        1000
#define MGIST_EXTRACT_GET_BOXES()   (PG_HAS_OPCLASS_OPTIONS() ? \
...
#define MGIST_EXTRACT_QX_DEFAULT    1000.0
#define MGIST_EXTRACT_QX_MAX        1000000.0
#define MGIST_EXTRACT_GET_QX()   (PG_HAS_OPCLASS_OPTIONS() ? \
          ((MGIST_QUERY_Options *) PG_GET_OPCLASS_OPTIONS())->qx : \
          MGIST_EXTRACT_QX_DEFAULT)

/* Average query height (in meters) */
#define MGIST_EXTRACT_QY_DEFAULT    1000.0
#define MGIST_EXTRACT_QY_MAX        1000000.0
#define MGIST_EXTRACT_GET_QY()   (PG_HAS_OPCLASS_OPTIONS() ? \
          ((MGIST_QUERY_Options *) PG_GET_OPCLASS_OPTIONS())->qy : \
          MGIST_EXTRACT_QY_DEFAULT)

/* Average query duration (in minutes) */
#define MGIST_EXTRACT_QT_DEFAULT    1000.0
#define MGIST_EXTRACT_QT_MAX        1000000.0
#define MGIST_EXTRACT_GET_QT()   (PG_HAS_OPCLASS_OPTIONS() ? \
          ((MGIST_QUERY_Options *) PG_GET_OPCLASS_OPTIONS())->qt : \
          MGIST_EXTRACT_QT_DEFAULT)
...
PG_FUNCTION_INFO_V1(Tpoint_mgist_box_options);
/**
 * ME-GiST options for temporal points
 */
PGDLLEXPORT Datum
Tpoint_mgist_box_options(PG_FUNCTION_ARGS)
{
  local_relopts *relopts = (local_relopts *) PG_GETARG_POINTER(0);

  init_local_reloptions(relopts, sizeof(MGIST_BOXES_Options));
  add_local_int_reloption(relopts, "k",
              "number of boxes for extract method",
              MGIST_EXTRACT_BOXES_DEFAULT, 1, MGIST_EXTRACT_BOXES_MAX,
              offsetof(MGIST_BOXES_Options, num_boxes));

  PG_RETURN_VOID();
}

PG_FUNCTION_INFO_V1(Tpoint_mgist_query_options);
/**
 * ME-GiST options for temporal points
 */
PGDLLEXPORT Datum
Tpoint_mgist_query_options(PG_FUNCTION_ARGS)
{
  local_relopts *relopts = (local_relopts *) PG_GETARG_POINTER(0);

  init_local_reloptions(relopts, sizeof(MGIST_QUERY_Options));
  add_local_real_reloption(relopts, "qx",
              "Average query width (in meters)",
              MGIST_EXTRACT_QX_DEFAULT, 1, MGIST_EXTRACT_QX_MAX,
              offsetof(MGIST_QUERY_Options, qx));
  add_local_real_reloption(relopts, "qy",
              "Average query height (in meters)",
              MGIST_EXTRACT_QY_DEFAULT, 1, MGIST_EXTRACT_QY_MAX,
              offsetof(MGIST_QUERY_Options, qy));
  add_local_real_reloption(relopts, "qt",
              "Average query duration (in minutes)",
              MGIST_EXTRACT_QT_DEFAULT, 1, MGIST_EXTRACT_QT_MAX,
              offsetof(MGIST_QUERY_Options, qt));
...
static Datum
tpoint_mgist_extract(FunctionCallInfo fcinfo, 
  STBox * (*tsequence_extract)(FunctionCallInfo fcinfo, 
...
tsequence_equisplit(FunctionCallInfo fcinfo, const TSequence *seq, int32 *nkeys)
{
  ...
  int32 count = MGIST_EXTRACT_GET_BOXES();
...
PG_FUNCTION_INFO_V1(Tpoint_mgist_equisplit);
/**
 * ME-GiST extract methods for temporal points
 */
PGDLLEXPORT Datum
Tpoint_mgist_equisplit(PG_FUNCTION_ARGS)
{
...
tsequence_mergesplit(FunctionCallInfo fcinfo, const TSequence *seq, int32 *nkeys)
{
  ...
  int *box_states;
  STBox *boxes, *result;
  int32 count = seq->count - 1, max_count = MGIST_EXTRACT_GET_BOXES();
  int i, k = 0;
...
PG_FUNCTION_INFO_V1(Tpoint_mgist_mergesplit);
/**
 * ME-GiST extract methods for temporal points
 */
PGDLLEXPORT Datum
...
tsequence_linearsplit(FunctionCallInfo fcinfo, const TSequence *seq, int32 *nkeys)
{
  ...
  int32 count = 0;
  double  qx = MGIST_EXTRACT_GET_QX(),
          qy = MGIST_EXTRACT_GET_QY(),
...
PG_FUNCTION_INFO_V1(Tpoint_mgist_linearsplit);
/**
 * ME-GiST extract methods for temporal points
 */
PGDLLEXPORT Datum
Tpoint_mgist_linearsplit(PG_FUNCTION_ARGS)
{
...
tsequence_manualsplit(FunctionCallInfo fcinfo, const TSequence *seq, int32 *nkeys)
{
  STBox *result;
  STBox box1;
  int i, k = 0, segs_per_split = MGIST_EXTRACT_GET_BOXES();
...
PG_FUNCTION_INFO_V1(Tpoint_mgist_manualsplit);
/**
 * ME-GiST extract methods for temporal points
 */
PGDLLEXPORT Datum
Tpoint_mgist_manualsplit(PG_FUNCTION_ARGS)
{
