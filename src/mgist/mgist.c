...
/*
 * Create and return a temporary memory context for use by ME-GiST. We
 ...
 * ME-GiST code itself, to avoid the need to do some awkward manual
 * memory management.
 */
MemoryContext
			 ...
			 IndexInfo *indexInfo)
{
	...
	if (mgiststate == NULL)
	{
		...
		indexInfo->ii_AmCache = mgiststate;
...
mgistfixsplit(GISTInsertState *state, MGISTSTATE *mgiststate)
{
	...
	for (;;)
	{
		...
		downlink = mgistformdownlink(state->r, buf, mgiststate, stack, state->is_build);
				  ...
				  MGISTSTATE *mgiststate, List *splitinfo, bool unlockbuf)
{
	...
	gistFindCorrectParent(state->r, stack, state->is_build);
	(void) mgistinserttuples(state, stack->parent, mgiststate,
							tuples, 2,
...
initMGISTstate(Relation index)
{
	 ...
	 */
	mgiststate->nonLeafTupdesc = CreateTupleDescTruncatedCopy(index->rd_att,
